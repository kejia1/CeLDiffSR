...
from ldm.modules.transformer.models import DiT_models
from ldm.modules.esrt_module import esrt
from ldm.modules.SWIFT_model.SWIFT import SWIFT as SWIFT_model
...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.model = DiffusionWrapper(unet_config, DiT_config, 
                                      conditioning_key,DiT=True)
    ...
    def training_step(self, batch, logger,optimizer, batch_idx=0,epoch=0):
        
        loss, loss_dict = self.shared_step(batch)

        # if epoch % 10==0:
            # logger.info("loss_simple: %s",loss_dict['train/loss_simple'].data)
            # logger.info("loss_vlb: %s",loss_dict['train/loss_vlb'].data)
            # if self.use_perceptual:
            #     logger.info("loss_perceptual: %s",loss_dict['train/perceptual_loss'].data)
            # logger.info("loss: %s",loss_dict['train/loss'].data)
        
            # logger.info("epoch: %s",epoch)
            # if self.use_scheduler:
            # lr = optimizer.param_groups[0]['lr']
            # logger.info("lr_abs: %s",lr)
...
class LatentDiffusionSR(DDPM):
    ...
    def __init__(self,
        ...
        ckpt_path = kwargs.pop("ckpt_path", None)
        ignore_keys = kwargs.pop("ignore_keys", [])
        super().__init__(unet_config=unet_config,DiT_config=DiT_config,ESRT_config=ESRT_config,SWIFT_config=SWIFT_config,conditioning_key=conditioning_key, *args, **kwargs)
        self.concat_mode = concat_mode
        self.cond_stage_trainable = cond_stage_trainable
        self.cond_stage_key = cond_stage_key
    ...
    def _dequeue_and_enqueue(self):
        ...
        if b == self.configs.data.batch_size:
    ...
    def get_input(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
        ...
        out = [z, x] # x 为退化后的低质量图片，z 为 x 经过vae encode 之后的潜在特征。
        out.append(z_gt) # z_gt 为gt经过vae encode之后的潜在特征。
    ...
    def p_losses(self, lq, cond, gt, t, noise=None):
        ...
        if self.noise_offset:
            noise+=self.noise_offset*torch.randn((noise.shape[0],noise.shape[1],1,1),device=noise.device)
            # print("Using noise_offset")
        if self.input_perturbation:
            noise+=self.input_perturbation*torch.randn_like(noise)
            # print("Using input_perturbation")
        ...
        if self.use_perceptual:
            perceptual_loss=self.criterion(model_output,target)
...
class DiffusionWrapper(pl.LightningModule):
    def __init__(self, DiT_config, conditioning_key,DiT=True):
        super().__init__()
        # self.diffusion_model = instantiate_from_config(diff_model_config)
        if DiT:
            self.diffusion_model=DiT_models["DiT-XL/8"](
                **DiT_config
            )
            print("Using DiT")
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None):
        if self.conditioning_key is None:
            ...
            # out = self.diffusion_model(x, t,c_concat[0])
...
class PerceptualLoss(nn.Module):
    def __init__(self):
        ...
        ckpt=torch.load("", # path of vgg
                        map_location='cpu')
        vgg_model.load_state_dict(ckpt)
        vgg=vgg_model.features
        self.vgg_layers = nn.Sequential(*list(vgg[:16]))  # 只使用VGG的前几层
    ...
    def forward(self, input_img, target_img):
        input_img_normalized = self.normalize(input_img)
        target_img_normalized = self.normalize(target_img)
        input_features = self.vgg_layers(input_img_normalized)
        target_features = self.vgg_layers(target_img_normalized)
        loss = torch.mean((input_features - target_features) ** 2)
        return loss
