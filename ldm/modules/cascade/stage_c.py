...
class StageC(nn.Module):
    def __init__(self, c_in=16, c_out=16, c_r=64, patch_size=1, c_cond=2048, c_hidden=[2048, 2048], nhead=[32, 32],
                 ...
                 c_clip_img=768, c_clip_seq=4, kernel_size=3,
    ...
    def gen_c_embeddings(self, clip_img):
        if len(clip_img.shape) == 2:
            clip_img = clip_img.unsqueeze(1)
        clip_img = self.clip_img_mapper(clip_img).view(clip_img.size(0), clip_img.size(1) * self.c_clip_seq, -1)
        clip = self.clip_norm(clip_img)
        return clip

    def _down_encode(self, x, r_embed, clip=None, cnet=None):
        level_outputs = []
    ...
    def _up_decode(self, level_outputs, r_embed, clip=None, cnet=None):
        x = level_outputs[0]
