from ldm.modules.esrt_module import common
import torch
...
class Un(nn.Module):
    def __init__(self,n_feats, wn,time_dim=None):
    ...
    def forward(self,x,t):
        ...
        out = x3
        b,c,h,w = x3.shape
        out = self.attention(self.reduce(torch.cat([x1,x2,x3],dim=1)),t)
...
class ESRT(nn.Module):
    def __init__(self, upscale=8, conv=common.default_conv):
        ...
        self.time_mlp = nn.Sequential(
            TimeEmbedding(n_feats),
            nn.Linear(n_feats, n_feats * 6),
            Swish(),
            nn.Linear(n_feats * 6, n_feats)
        )
        
        # define head module
        modules_head = [conv(6, n_feats, kernel_size)]
        
        # define body module
        modules_body = nn.ModuleList()
        for i in range(n_blocks):
            modules_body.append(
                Un(n_feats=n_feats, wn = wn,time_dim=time_dim))

        # define tail module
        modules_tail = [
            common.Upsampler(conv, scale, n_feats, act=False),
            conv(n_feats, 6, kernel_size)]


        self.up = nn.Sequential(common.Upsampler(conv,scale,n_feats,act=False),
                          BasicConv(in_planes=n_feats,out_planes=6,kernel_size=3,padding=1,stride=1))
        
        self.down=nn.Sequential(common.Downsampler(scale,6,act=False))
        
        self.head = nn.Sequential(*modules_head)
        self.body = nn.Sequential(*modules_body)
        self.tail = nn.Sequential(*modules_tail)
        self.reduce = conv(n_blocks*n_feats, n_feats, kernel_size)
        self.conv=conv(6,3,3,1,1)

    
    
    def forward(self, x1, timesteps= None, test=False):
        # x1 = self.sub_mean(x1)
